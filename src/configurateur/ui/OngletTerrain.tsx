  ...
  return (
    <div>
      ...
      <div className="flex gap-2 items-center">
        ...
        <input type="number" min={TERRAIN_MIN} max={TERRAIN_MAX} step={0.5} value={valeur} onChange={(e) => onChange(Number(e.target.value))} className={`${input} w-20`} aria-label={`${label} en mètres`} />
      </div>
    </div>
  )
}
